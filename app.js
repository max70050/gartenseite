...
const resetCheckoutModalState = () => {
    // Verstecke den Stripe-Container und zeige den "Mit Karte zahlen"-Button
    paymentElementContainer.style.display = 'none';
    startOnlinePaymentBtn.style.display = 'flex'; // 'flex' wegen der Zentrierung im CSS

    // Setze den "Mit Karte zahlen"-Button zurück
...
startOnlinePaymentBtn.addEventListener('click', initializeCheckout);
submitPaymentBtn.addEventListener('click', handleSubmit);

async function initializeCheckout() {
    // Elemente für den Button holen
    const btnText = startOnlinePaymentBtn.querySelector('.btn-text');
    const btnSpinner = startOnlinePaymentBtn.querySelector('.btn-spinner');

    // Spinner anzeigen und Button deaktivieren
    startOnlinePaymentBtn.disabled = true;
    ...
    try {
        ...
        // Wenn erfolgreich, blende den initialen Button aus und zeige die Bezahloptionen
        startOnlinePaymentBtn.style.display = 'none';
        ...
        resetCheckoutModalState(); // WICHTIG: Setzt den Button zurück, wenn ein Fehler auftritt
    }
}
...
async function checkStatus() {
    ...
    switch (paymentIntent.status) {
        case "succeeded":
            // KORREKTUR: Lade die Bestelldaten aus dem Speicher
            ...
            if (pendingOrderJSON) {
                ...
                localStorage.removeItem('pendingOrderDetails'); // Aufräumen
...
async function saveOrderToFirestore(paymentMethod, orderCart, orderPickupDay, orderPickupTime) {
    ...
    try {
        ...
        confirmOrderBtn.textContent = 'Bar an der Kasse zahlen';
    }
}
...
function resetPaymentButton() {
    startOnlinePaymentBtn.textContent = 'Jetzt online bezahlen';
    startOnlinePaymentBtn.disabled = false;
}
// =================================================================
// ENDE: STRIPE ONLINE-ZAHLUNG
// =================================================================
